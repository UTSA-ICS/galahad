        ...
        while( not self.exit_requested ):
            ...
            for v in virtues:
                ...
                if( aws_inst_id == None ):
                    ...
                    print( 'Virtue was not found on AWS: {0}.'.format( v['ipAddress'] ) )
                    #self.inst.del_obj( 'cid', v['id'], objectClass='OpenLDAPvirtue' )

                vm_state = ec2.describe_instances( InstanceIds=[aws_inst_id] )['Reservations'][0]['Instances'][0]['State']['Name']

                if( aws_state_to_virtue_state[ vm_state ] != v['state'] ):
                    # Update LDAP's virtue object
                    print( 'Virtue state has changed: {0} {1} -> {2}'.format( v['id'], v['state'], aws_state_to_virtue_state[ vm_state ] ) )
            ...
            for r in roles:
                if( roles_dict[ r['id'] ] < BACKUP_VIRTUE_COUNT ):
                    thr = CreateVirtueThread( self.inst.email, self.inst.password, r['id'], role=r )
                    thr.start()
...
class CreateVirtueThread(threading.Thread):
    ...
    def run( self ):
        ...
        if( self.role == None ):
            role = self.inst.get_obj( 'cid', self.role_id, objectClass='OpenLDAProle' )
        ...
        aws = AWS()
        instance = aws.instance_create( image_id=ami_id,
                                        inst_type='t2.small',
