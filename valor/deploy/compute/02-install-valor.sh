...
"
sudo dpkg -i --root="${PATH_TO_FILESYSTEM_DIR}" ${XEN_PACKAGES}
sudo chroot "${PATH_TO_FILESYSTEM_DIR}" ldconfig /usr/local/lib

#
# Configure XEN system files
#
sudo cp "${CONFIG_DIR}"/vif-bridge "${PATH_TO_FILESYSTEM_DIR}"/etc/xen/scripts/vif-bridge
sudo cp "${CONFIG_DIR}"/xen-network-common.sh "${PATH_TO_FILESYSTEM_DIR}"/etc/xen/scripts/xen-network-common.sh
sudo cp "${CONFIG_DIR}"/xl.conf "${PATH_TO_FILESYSTEM_DIR}"/etc/xen/xl.conf
sudo chroot "${PATH_TO_FILESYSTEM_DIR}" systemctl enable xencommons

#
# Install introspection packages and files
#
sudo cp "${CONFIG_DIR}"/libvmi/libvmi.conf "${PATH_TO_FILESYSTEM_DIR}"/etc/libvmi.conf
sudo mkdir -p "${PATH_TO_FILESYSTEM_DIR}"/usr/share/libvmi/kernel-data
sudo cp "${CONFIG_DIR}"/libvmi/kernel-data/System.map-4.13.0-46-generic "${PATH_TO_FILESYSTEM_DIR}"/usr/share/libvmi/kernel-data/System.map-4.13.0-46-generic

#
# Configure tty
#
sudo cp "${CONFIG_DIR}"/hvc0.conf /etc/init/
